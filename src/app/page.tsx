...
import { useState, useRef, MouseEvent } from 'react';import { Button } from '@/components/ui/button';import { Card } from '@/components/ui/card';import { Upload, RefreshCw, Trash2, Plus } from 'lucide-react';

interface Box {id: string;x: number;y: number;width: number;height: number;type: 'WG' | 'CG'; // Box type: WG (Whole Gland) or CG (Central Gland)}

interface SegmentationResult {success: boolean;masks?: {WG?: string;  // Whole Gland maskCG?: string;  // Central Gland maskPZ?: string;  // Peripheral Zone mask (WG - CG)};error?: string;}

export default function MedicalSAMDemo() {const [image, setImage] = useState<string | null>(null);const [boxes, setBoxes] = useState<Box[]>([]); // Multiple boxes supportconst [selectedBoxType, setSelectedBoxType] = useState<'WG' | 'CG'>('WG'); // Default to WGconst [isDrawing, setIsDrawing] = useState(false);const [startPoint, setStartPoint] = useState<{ x: number; y: number } | null>(null);const [currentBox, setCurrentBox] = useState<Box | null>(null);const [isLoading, setIsLoading] = useState(false);const [result, setResult] = useState<SegmentationResult | null>(null);const [imageDimensions, setImageDimensions] = useState<{ width: number; height: number } | null>(null);const [useMedicalMode, setUseMedicalMode] = useState(true);const fileInputRef = useRef(null);const canvasRef = useRef(null);

// Use ref to track drawing state synchronouslyconst isDrawingRef = useRef(false);const currentBoxRef = useRef<Box | null>(null);

// Generate unique ID for boxesconst generateBoxId = () => box-${Date.now()}-${Math.random().toString(36).substr(2, 9)};

// Load sample image for quick testingconst loadSampleImage = () => {const sampleImage = new Image();sampleImage.onload = () => {setImageDimensions({ width: sampleImage.width, height: sampleImage.height });setImage('/assets/test_image.png');setBoxes([]); // Clear all boxessetResult(null);setStartPoint(null);setCurrentBox(null);};sampleImage.onerror = () => {console.error('Failed to load sample image');alert('Failed to load sample image');};sampleImage.src = '/assets/test_image.png';};

const handleImageUpload = (e: React.ChangeEvent) => {const file = e.target.files?.[0];if (file) {const reader = new FileReader();reader.onload = (event) => {const img = new Image();img.onload = () => {setImageDimensions({ width: img.width, height: img.height });setImage(event.target?.result as string);setBoxes([]); // Clear all boxessetResult(null);setStartPoint(null);setCurrentBox(null);};img.src = event.target?.result as string;};reader.readAsDataURL(file);}};

const handleMouseDown = (e: MouseEvent) => {if (!image || !imageDimensions) return;
...
const handleMouseMove = (e: MouseEvent) => {if (!isDrawingRef.current || !startPoint || !imageDimensions) return;
...
const handleMouseUp = () => {console.log('[MouseUp] isDrawingRef:', isDrawingRef.current, 'currentBoxRef:', currentBoxRef.current);
...
const handleMouseLeave = () => {console.log('[MouseLeave] isDrawingRef:', isDrawingRef.current, 'currentBoxRef:', currentBoxRef.current);
...
// Function to delete a specific boxconst deleteBox = (boxId: string) => {setBoxes(prev => prev.filter(box => box.id !== boxId));setResult(null); // Clear results when boxes change};

// Function to clear all boxesconst clearAllBoxes = () => {setBoxes([]);setResult(null);};

const handleSegment = async () => {if (!image || boxes.length === 0) {console.warn('No image or boxes provided');return;}
...
const resetAll = () => {setImage(null);setBoxes([]);setResult(null);setImageDimensions(null);setStartPoint(null);setCurrentBox(null);if (fileInputRef.current) {fileInputRef.current.value = '';}};

// Combine boxes with current drawing box for displayconst displayBoxes = [...boxes, ...(currentBox && isDrawing ? [currentBox] : [])];

return ({/* Shenzhen Hetao College Logo - absolute top-right */}
    ...
    <div className="grid gap-6 lg:grid-cols-[460px_1fr]">
      ...
      <div className="space-y-6">
        {/* Upload Section */}
        <Card className="p-7 bg-gradient-to-br from-white to-blue-50/50 dark:from-slate-900 dark:to-blue-950/20 border-blue-200 dark:border-blue-800">
          ...
          <div className="space-y-2.5">
            ...
            <label htmlFor="image-upload">
              <Button
                variant="outline"
                className="w-full h-14 text-xl border-blue-300 hover:border-blue-400 hover:bg-blue-50 dark:border-blue-700 dark:hover:bg-blue-950/30 transition-all rounded-xl border-blue-300 hover:border-blue-400 hover:bg-blue-50 dark:border-blue-700 dark:hover:bg-blue-950/30 transition-all"
                asChild
              >
            ...
            {image && (
              <Button
                variant="ghost"
                onClick={resetAll}
                className="w-full h-14 text-xl rounded-xl text-slate-600 text-slate-600 hover:text-slate-900 dark:text-slate-400 dark:hover:text-slate-100"
              >
        ...
        {/* Box Type Selection */}
        <Card className="p-7 bg-gradient-to-br from-white to-orange-50/50 dark:from-slate-900 dark:to-orange-950/20 border-orange-200 dark:border-orange-800">
          <div className="flex items-center gap-2 mb-3">
          ...
          <div className="space-y-2">
            <label className="flex items-center space-x-3 p-5 rounded-lg border-2 cursor-pointer transition-all hover:border-blue-400 hover:bg-blue-50/50 dark:hover:bg-blue-950/20 dark:border-blue-900"
              style={{
              ...
              <span className="flex-1">
                ...
                <span className="ml-2 text-xl text-slate-600 dark:text-slate-400">全腺体</span>
              </span>
            </label>
            <label className="flex items-center space-x-3 p-5 rounded-lg border-2 cursor-pointer transition-all hover:border-orange-400 hover:bg-orange-50/50 dark:hover:bg-orange-950/20 dark:border-orange-900"
              style={{
              ...
              <span className="flex-1">
                ...
                <span className="ml-2 text-xl text-slate-600 dark:text-slate-400">中央腺体</span>
              </span>
            </label>
        ...
        {/* Instructions */}
        <Card className="p-7 bg-gradient-to-br from-white to-purple-50/50 dark:from-slate-900 dark:to-purple-950/20 border-purple-200 dark:border-purple-800">
          <div className="flex items-center gap-2 mb-3">
          ...
          <ul className="space-y-2 text-xl text-slate-600 dark:text-slate-400">
            <li className="flex items-start gap-2">
              <span className="mt-0.5 flex-shrink-0 w-8 h-8 rounded-full bg-gradient-to-br from-blue-500 to-cyan-500 text-white text-xs flex items-center justify-center font-bold">1</span>
              <span>上传医学图像</span>
            </li>
            <li className="flex items-start gap-2">
              <span className="mt-0.5 flex-shrink-0 w-8 h-8 rounded-full bg-gradient-to-br from-cyan-500 to-green-500 text-white text-xs flex items-center justify-center font-bold">2</span>
              <span>选择标注类型 (WG/CG)</span>
            </li>
            <li className="flex items-start gap-2">
              <span className="mt-0.5 flex-shrink-0 w-8 h-8 rounded-full bg-gradient-to-br from-green-500 to-yellow-500 text-white text-xs flex items-center justify-center font-bold">3</span>
              <span>在图像上绘制标注框</span>
            </li>
            <li className="flex items-start gap-2">
              <span className="mt-0.5 flex-shrink-0 w-8 h-8 rounded-full bg-gradient-to-br from-yellow-500 to-orange-500 text-white text-xs flex items-center justify-center font-bold">4</span>
              <span>运行分割</span>
            </li>
          </ul>
        </Card>
        ...
        <Card className="p-7">
          <h2 className="mb-4 text-2xl font-bold">模式选择</h2>
      ...
      <div className="lg:col-span-3">
        <Card className="p-6 rounded-3xl shadow-xl bg-gradient-to-br from-white to-slate-50 dark:from-slate-900 dark:to-slate-950/50 shadow-lg">
          {!image ? (
            ...
            <div className="space-y-4">
                      ...
                      >
                        ...
                        {displayBox.type === 'WG' ? (
                          <>
                            ...
                            {/* Box label */}
                            <div className="absolute -top-6 left-0 rounded bg-blue-500 px-2 py-0.5 text-2xl font-bold text-white">
                              WG
                            </div>
                          </>
                        ) : (
                          <>
                            ...
                            {/* Box label */}
                            <div className="absolute -top-6 left-0 rounded bg-orange-500 px-2 py-0.5 text-2xl font-bold text-white">
                              CG
                            </div>
                          </>
                        )}
              ...
              <div className="flex items-center justify-between">
                ...
                <Button
                  ...
                  className="min-w-[240px] h-14 text-xl font-bold rounded-2xl"
                >
                  {isLoading ? '处理中...' : '运行分割'}
                </Button>
              </div>
